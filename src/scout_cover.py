...
H, W =  10, 10
scout = [ [ Int("scout_%s%s" % (r, c)) for c in range(W) ] for r in range(H) ]
num_scouts = Sum([ scout[r][c] for r in range(H) for c in range(W) ])
num_scout_pairs = Int('k')
squares_are_allowed = [ Or(0 == scout[r][c], scout[r][c] == 1) for r in range(H) for c in range(W) ]
lakes_are_forbidden = [ scout[r][c] == 0 for r in range(4, 6) for c in chain(range(2, 4), range(6, 8)) ]

# http://forum.stratego.com/topic/1134-stratego-quizz-and-training-forum/?p=11670

# Cache the row and column sum into variables.
num_scouts_per_open_row           = [ Sum([ scout[r][c] for c in range(W)    ]) for r in chain(range(0, 4), range(6, H)) ]
num_scouts_per_row_in_left_lane   = [ Sum([ scout[r][c] for c in range(0, 2) ]) for r in range(4, 6) ]
num_scouts_per_row_in_center_lane = [ Sum([ scout[r][c] for c in range(4, 6) ]) for r in range(4, 6) ]
num_scouts_per_row_in_right_lane  = [ Sum([ scout[r][c] for c in range(8, W) ]) for r in range(4, 6) ]
num_scouts_per_open_column        = [ Sum([ scout[r][c] for r in range(H)    ]) for c in chain(range(0, 2), range(4, 6), range(8, W)) ]
num_scouts_per_column_below_lakes = [ Sum([ scout[r][c] for r in range(0, 4) ]) for c in chain(range(2, 4), range(6, 8)) ]
num_scouts_per_column_above_lakes = [ Sum([ scout[r][c] for r in range(6, H) ]) for c in chain(range(2, 4), range(6, 8)) ]
...
def search_solution(N, verbose = None):
    if verbose != None:
    ...
    if problem.check() == sat:
        ...
        f = np.array([ [ '2' if m.evaluate(scout[r][c]) == 1 else ('#' if r in range(4, 6) and c in chain(range(2, 4), range(6, 8)) else '.') for c in range(W) ] for r in range(H) ]).reshape(H, W)
...
search_solution(9)  # takes ~30 seconds
